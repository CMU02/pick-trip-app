...
| 2 | Login | 카카오 로그인 완료 후 앱 복귀 | **미검증** — 계정 자격 증명 필요 |
| 3 | Login | 구글 로그인 완료 후 앱 복귀 | **미검증** — 계정 자격 증명 필요 |
...
| 7 | Main / Profile | 취향 설정, 로그아웃 후 화면 리셋 | **취향 토글 정상.** 로그아웃 버튼은 게스트 상태에서도 노출됨. 로그아웃 후 리셋은 미검증 |
...
| 10 | Itinerary | 일정 생성, 결과 표시, 저장 | **생성·표시 정상** (1박 2일, 1일차/2일차 분배). 저장은 게스트라 `로그인하고 일정 저장`으로 유도됨 — 미검증. 우선순위 반영에 불일치 있음 (아래 참조) |
| 11 | Resume | 저장된 일정 이어하기 | **미검증** — 10의 저장이 선행 조건 |
| 12 | Shared | 공유 시트 열림. 딥링크는 **콜드스타트**(앱 완전 종료 후 링크 진입)와 **웜스타트**(백그라운드 상태에서 링크 진입)를 따로 기록 | **미검증** — 로그인 필요 |
| 13 | 세션 만료 | SecureStore의 토큰을 무효값으로 바꾼 뒤 탐색 목록을 새로고침 → Alert → Auth 화면으로 리셋되는지, 뒤로가기로 이전 화면에 못 돌아가는지 | **부분 관찰** — 4번에서 의도치 않게 재현됨 (아래 참조). 명시적 토큰 무효화 검증은 미실시 |
